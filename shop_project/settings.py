...
SECRET_KEY = os.environ.get('SECRET_KEY', 'django-insecure-r@)8j9p&fflj2&a@h*1p!b9=*0wc%&h-ge_@as4+smj*4n$=2q')
...
# Для разработки используем SQLite, для производства PostgreSQL через DATABASE_URL
if DEBUG:
    DATABASES = {
        "default": {
    ...
    # Production: использовать PostgreSQL из переменной окружения DATABASE_URL
    DATABASES = {
...
# These settings are only enabled when DEBUG=False for security
if not DEBUG:
    SECURE_BROWSER_XSS_FILTER = True
    SECURE_CONTENT_TYPE_NOSNIFF = True
    SECURE_HSTS_INCLUDE_SUBDOMAINS = True
    SECURE_HSTS_SECONDS = 31536000  # 1 год - сообщает браузерам использовать только HTTPS
    SECURE_HSTS_PRELOAD = True  # Позволяет добавить сайт в HSTS preload list
    SECURE_REDIRECT_EXEMPT = []
    SESSION_COOKIE_SECURE = True  # Куки только через HTTPS
    CSRF_COOKIE_SECURE = True  # CSRF токен только через HTTPS
    SECURE_SSL_REDIRECT = True  # Автоматическое перенаправление на HTTPS
